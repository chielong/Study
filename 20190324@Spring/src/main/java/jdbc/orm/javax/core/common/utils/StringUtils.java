...
public class StringUtils {
    ...
    /**
     * 将形如 key1=value1,key2=value2 的字符串，转化为Map
     *
     * @param str
     * @param splitString
     * @return
     ...
     * @return
     */
    private static Map<String, String> strToMap(String str, String splitString, String linkStr) {
        ...
        for(int i = 0 ; i < values.length ; i++) {
            ...
            int pos = tmpValue.indexOf(linkStr);
            String key = "";
            String value = "";

            if(pos > -1) {
                key = tmpValue.substring(0 , pos);
                value = tmpValue.substring(pos + splitString.length());
            } else {
                key = tmpValue;
            }
     ...
     * @return
     */
    private static String pagination(String text, int pageIndex, int pageSize, int lineLength) {
        if(isEmpty(text)) {
            return text;
        }
        ...
        String result = text.toString().substring(
                (pageIndex - 1) * pageSize ,
                pageIndex * pageSize >= text.length() - 1 ? text.length() : pageIndex * pageSize);
     ...
     *              中文字符串或者英文字符串，或者null
     * @return
     *              返回字符串byte数组长度，如果是空字符串则返回0，如果是null则返回-1.
     */
     ...
     * @return 返回全部替换后的字符串
     */
    public static String replace(String str, char searchStr, String replaceStr) {
        return replace(str, searchStr + "", replaceStr, true);
    }
    ...
    public static String removeHtml(String content) {
        ...
        try {
            pHtml = Pattern.compile("<[^>]+>" , Pattern.CASE_INSENSITIVE);
            mHtml = pHtml.matcher(content);
    ...
    public static String encodeBase64(String str) {
        ...
        if (b != null) {
            return Base64.getEncoder().encode(b).toString();
        }
        return null;
    }
